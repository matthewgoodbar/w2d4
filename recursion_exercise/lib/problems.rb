...
def sum_array(array)
    ...
    len = array.length
    return sum_array(array[0...len/2]) + sum_array(array[len/2..-1])
end
...
def reverse_string(str)
    ...
    len = str.length
    return reverse_string(str[len/2..-1]) + reverse_string(str[0...len/2])
end
...
def flatten(data)
    ...
    return data.inject([]) {|acc, el| acc += flatten(el)}
end