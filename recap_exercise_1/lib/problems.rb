...
def find_bigrams(str, bigrams)
    ...
    bigrams.each do |bigram|
        (0...(str.length-1)).each do |i|
            res << bigram if str[i] == bigram[0] && str[i+1] == bigram[1]
        end
    end
    return res
end
...
class String
    ...
    def substrings(length = nil)
        res = []
        (0...self.length).each do |outer_i|
            (0..outer_i).each do |inner_i|
                res << self[inner_i..outer_i] if length ? outer_i - inner_i == length-1 : true
            end
        end
        return res
    end
